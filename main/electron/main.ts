...
function ensureExecutable(candidate: string | null | undefined) {
  ...
  try {
    ...
    if (process.platform !== "win32") {
      const needed = 0o111;
      if ((stats.mode & needed) !== needed) {
        fs.chmodSync(candidate, stats.mode | needed);
      }
    }
  } catch (error) {
    console.warn("[binary] failed to ensure executable:", candidate, error);
  }
  return candidate;
}
...
function getBundledBinaryEnv(): NodeJS.ProcessEnv {
  ...
  const ffmpegPath = ensureExecutable(
    process.env.FRAMESCRIPT_FFMPEG_PATH ?? resolveBundledBinaryPath(ffmpegInstaller),
  );
  const ffprobePath = ensureExecutable(
    process.env.FRAMESCRIPT_FFPROBE_PATH ?? resolveBundledBinaryPath(ffprobeInstaller),
  );
  const chromiumPath =
    process.env.FRAMESCRIPT_CHROMIUM_PATH ??
    process.env.PUPPETEER_EXECUTABLE_PATH ??
    ensureExecutable(resolvePuppeteerExecutablePath());
  if (ffmpegPath) {
