...
fn parse_boolish_env(var: &str) -> Option<bool> {
    std::env::var(var)
        .ok()
        .and_then(|value| {
...
fn should_disable_chromium_sandbox() -> bool {
    if let Some(flag) = parse_boolish_env("FRAMESCRIPT_RENDER_NO_SANDBOX")
        .or_else(|| parse_boolish_env("CHROMIUM_NO_SANDBOX"))
    {
        return flag;
    }
...
fn parse_browser_args_env(var: &str) -> Vec<String> {
    std::env::var(var)
        .ok()
        .map(|value| {
            value
...
async fn spawn_browser_instance(
    ...
    let disable_sandbox = should_disable_chromium_sandbox();
    ...
    extra_args.extend(parse_browser_args_env("CHROMIUM_FLAGS"));
    extra_args.extend(parse_browser_args_env("CHROMIUM_EXTRA_ARGS"));
    extra_args.extend(parse_browser_args_env("FRAMESCRIPT_CHROMIUM_ARGS"));

    if disable_sandbox && !extra_args.iter().any(|arg| arg == "--no-sandbox") {
