...
export const sound_length = (sound: Sound | string): number => {
  ...
  if (soundLengthCache.has(resolved.path)) {
    const cached = soundLengthCache.get(resolved.path)!
    if (cached > 0) {
      return cached
    }
  }
  ...
  try {
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      soundLengthCache.delete(resolved.path)
      return 0
    }
  } catch (error) {
...
export const Sound = ({ sound, trim }: SoundProps) => {
  ...
  const [bufferDurationFrames, setBufferDurationFrames] = useState<number | null>(null)
  const rawDurationFrames = useMemo(
    () => sound_length(resolvedSound),
    [resolvedSound],
  )
  const effectiveDurationFrames =
    rawDurationFrames > 0 ? rawDurationFrames : bufferDurationFrames ?? 0
  ...
  const durationFrames = Math.max(
    0,
    effectiveDurationFrames - trimStartFrames - trimEndFrames,
  )
  ...
  useEffect(() => {
    if (rawDurationFrames > 0 || bufferDurationFrames != null) return
    let cancelled = false
    ...
    const measure = async () => {
      try {
        ...
        if (cancelled) return
        const frames = Math.round(buffer.duration * PROJECT_SETTINGS.fps)
        if (frames > 0) {
          setBufferDurationFrames(frames)
          soundLengthCache.set(resolvedSound.path, frames)
        }
      } catch (error) {
        console.error("Sound: failed to measure duration from buffer", error)
      }
    }
  ...
  }, [bufferDurationFrames, ensureAudioContext, rawDurationFrames, resolvedSound.path])
